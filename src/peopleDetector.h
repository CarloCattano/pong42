...
public:
	...
	vector<ofRectangle> detect(cv::Mat img) {
		...
		if (img.cols < hog.winSize.width || img.rows < hog.winSize.height) {
			std::cerr << "[warning] Input image too small for HOG winSize — skipping detection.\n";
			return {};
		}
		...
		for (auto &rect : found) {
			rect =
				cv::Rect(rect.x * imageScale, rect.y * imageScale, rect.width * imageScale, rect.height * imageScale);
